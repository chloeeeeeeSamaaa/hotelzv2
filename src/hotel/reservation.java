...
public class reservation extends javax.swing.JFrame {
    ...
    public void autoID() {
        ...
        if (rs.next()) {
            ...
            System.out.println("maxID: " + maxID);  // Debugging the value of maxID

            if (maxID == null || maxID.length() <= 1) {
                jLabel12.setText("R");
                System.out.println("No existing reservations. Setting ReserveNo to R0001.");
            } else {
                // Safely extract the numeric part of the ID
                String numericPart = maxID.substring(1); // Skip the "R"
                long id = Long.parseLong(numericPart);
                id++;
                String newID = "R" + String.format("%04d", id);
                jLabel12.setText(newID);
                System.out.println("New ReserveNo: " + newID);  // Debugging the new ID
            }
        } else {
            System.out.println("No data found in the reservation table.");
        }
    } catch (SQLException ex) {
        Logger.getLogger(room.class.getName()).log(Level.SEVERE, null, ex);
    }
    ...
    if (txtcheckin.getDate() == null || txtcheckout.getDate() == null || 
        ...
        txtro.getSelectedIndex() == -1 || txtamount.getText().trim().isEmpty()) {
   ...
   try {
    pat = con.prepareStatement("SELECT * FROM reservation WHERE roomNo = ? AND userID = ? AND (CheckIn <= ? AND CheckOut >= ?)");
    pat.setString(1, roomNo);    // Set the room number
    pat.setInt(2, Login.UID);    // Set the user ID (important to prevent other users from accessing each other's data)
    pat.setString(3, EndDate);   // Set the end date
    pat.setString(4, StartDate); // Set the start date

    ResultSet rs = pat.executeQuery();

    if (rs.next()) {
        // Room is already booked for the selected dates
        JOptionPane.showMessageDialog(this, "Room is already taken for the selected dates.", "Room Unavailable", JOptionPane.ERROR_MESSAGE);
        return; // Exit the method if room is already booked
    }

    // Proceed to insert the reservation
    pat = con.prepareStatement("INSERT INTO reservation (ReserveNO, Name, Address, MobileNo, CheckIn, CheckOut, bedType, roomNo, roomType, amount, userID) VALUES (?,?,?,?,?,?,?,?,?,?,?)");
    ...
    pat.setInt(11, Login.UID); // Ensure the correct user ID is used
  ...
  public void Load_reservation() {
        ...
        try {
            pat = con.prepareStatement("SELECT * FROM reservation WHERE userID = '"+Login.UID+"'");
            ResultSet rs = pat.executeQuery();
